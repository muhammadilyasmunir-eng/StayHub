...
def _ensure_after_checkout(reservation: Reservation):
    if reservation.status in (ReservationStatus.CANCELLED, ReservationStatus.NO_SHOW):
        raise HTTPException(400, "Only a completed guest stay can be reviewed")
    if not reservation.check_out or date.today() < reservation.check_out:
        raise HTTPException(400, "The Review option becomes available after check-out")
...
def customer_review(reservation_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_customer)):
    ...
    review = db.query(GuestReview).filter(GuestReview.reservation_id == reservation.id).first()
    return {"eligible": bool(date.today() >= reservation.check_out and reservation.status not in (ReservationStatus.CANCELLED, ReservationStatus.NO_SHOW)), "review": _payload(review) if review else None}
