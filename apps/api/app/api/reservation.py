...
def _reservation_facilities(room_type,hotel):
    """Return currently saved room/property facilities and breakfast options."""
    names=[]
    for facility in (getattr(room_type,"facilities",[]) if room_type else []):
        if getattr(facility,"available",True) and getattr(facility,"name",None): names.append(str(facility.name).strip())
    for facility in (getattr(hotel,"facilities",[]) if hotel else []):
        if getattr(facility,"available",True) and getattr(facility,"name",None): names.append(str(facility.name).strip())
    unique=[]; seen=set()
    for name in names:
        key=name.casefold()
        if name and key not in seen: seen.add(key); unique.append(name)
    meals=[]
