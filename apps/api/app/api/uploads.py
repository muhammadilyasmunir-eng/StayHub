...
async def upload_hotel_photo(hotel_id: int, file: UploadFile = File(...), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    extension = _validate_file(file)
    ...
    if photo_count >= MAX_PROPERTY_PHOTOS: raise HTTPException(status_code=400, detail="Maximum 50 property photos are allowed.")
    url, size = await _save_file(file, extension)
    photo = HotelPhoto(hotel_id=hotel.id, photo_url=url, caption=file.filename, category="property", is_primary=False, sort_order=photo_count)
    db.add(photo); db.commit(); db.refresh(photo)
    return {"url": url, "filename": file.filename, "content_type": file.content_type, "size": size, "photo_id": photo.id, "photo_count": photo_count + 1, "max_photos": MAX_PROPERTY_PHOTOS}
...
async def upload_hotel_building_photo(hotel_id: int, file: UploadFile = File(...), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if existing is None and photo_count >= MAX_PROPERTY_PHOTOS: raise HTTPException(status_code=400, detail="Maximum 50 property photos are allowed.")
    url, size = await _save_file(file, extension)
    if existing is None:
        existing = HotelPhoto(hotel_id=hotel.id, photo_url=url, caption=file.filename, category="building", is_primary=True, sort_order=0); db.add(existing)
    else:
        existing.photo_url=url; existing.caption=file.filename; existing.category="building"; existing.is_primary=True; existing.sort_order=0
    for other in hotel.photos:
        if other.id != getattr(existing, "id", None): other.is_primary=False
...
async def set_primary_hotel_photo(hotel_id: int, photo_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    hotel = db.query(Hotel).filter(Hotel.id == hotel_id).first()
    if hotel is None: raise HTTPException(status_code=404, detail="Hotel not found")
    _require_hotel_access(hotel, current_user)
    ...
    for other in hotel.photos:
        ...
        if other.id == photo.id: other.category="building"
        elif other.category == "building": other.category="property"
    photo.is_primary=True; photo.category="building"; photo.sort_order=0
    for index, other in enumerate(sorted(hotel.photos, key=lambda item: (0 if item.id == photo.id else 1, item.sort_order, item.id))):
        if other.id != photo.id: other.sort_order=index
...
async def delete_hotel_photo(hotel_id: int, photo_id: int, current_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    photo = db.query(HotelPhoto).filter(HotelPhoto.id == photo_id, HotelPhoto.hotel_id == hotel_id).first()
    if photo is None: raise HTTPException(status_code=404, detail="Property photo not found")
    was_primary=photo.is_primary; db.delete(photo); db.flush()
    if was_primary:
        replacement=db.query(HotelPhoto).filter(HotelPhoto.hotel_id==hotel_id).order_by(HotelPhoto.sort_order,HotelPhoto.id).first()
        if replacement: replacement.is_primary=True; replacement.category="building"; replacement.sort_order=0
    db.commit()
...
async def replace_hotel_photo(hotel_id: int, photo_id: int, file: UploadFile = File(...), current_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    extension=_validate_file(file)
    if extension not in IMAGE_EXTENSIONS: raise HTTPException(status_code=400, detail="Property photo must be JPG, JPEG, PNG, or GIF/WEBP.")
    photo=db.query(HotelPhoto).filter(HotelPhoto.id==photo_id,HotelPhoto.hotel_id==hotel_id).first()
    if photo is None: raise HTTPException(status_code=404, detail="Property photo not found")
    url,size=await _save_file(file,extension); photo.photo_url=url; photo.caption=file.filename; db.commit(); db.refresh(photo)
...
async def replace_hotel_document(hotel_id: int, document_id: int, file: UploadFile = File(...), current_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    url,size=await _save_file(file,extension); document.document_url=url; db.commit(); db.refresh(document)
    return {"url":url,"filename":file.filename,"content_type":file.content_type,"size":size,"document_id":document.id}


@router.delete("/hotel/{hotel_id}/document/{document_id}")
async def delete_hotel_document(hotel_id: int, document_id: int, current_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    document=db.query(HotelDocument).filter(HotelDocument.id==document_id,HotelDocument.hotel_id==hotel_id).first()
    if document is None: raise HTTPException(status_code=404, detail="Registration document not found")
    db.delete(document); db.commit()
    return {"message":"Registration document removed.","document_id":document_id}
...
@router.delete("/hotel/{hotel_id}/owner-document/{document_kind}")
async def delete_owner_document(hotel_id: int, document_kind: str, current_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    if document_kind not in {"cnic_front","cnic_back","signed_agreement"}: raise HTTPException(status_code=400, detail="Unsupported owner document type")
    hotel=db.query(Hotel).filter(Hotel.id==hotel_id).first()
    if hotel is None: raise HTTPException(status_code=404, detail="Hotel not found")
    if document_kind=="cnic_front": hotel.owner_cnic_front_url=None
    elif document_kind=="cnic_back": hotel.owner_cnic_back_url=None
    else: hotel.signed_agreement_url=None
    db.commit(); db.refresh(hotel)
    return {"message":"Owner verification document removed.","document_kind":document_kind}
