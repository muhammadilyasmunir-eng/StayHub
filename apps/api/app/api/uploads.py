...
async def replace_hotel_photo(
    ...
    extension = _validate_file(file)
    if extension not in IMAGE_EXTENSIONS:
        raise HTTPException(status_code=400, detail="Property photo must be JPG, JPEG, PNG, WEBP, or GIF.")
...
async def replace_hotel_document(
    hotel_id: int,
    document_id: int,
    file: UploadFile = File(...),
    current_user: User = Depends(require_admin),
...
async def replace_owner_document(
    hotel_id: int,
    document_kind: str,
    file: UploadFile = File(...),
    current_user: User = Depends(require_admin),
