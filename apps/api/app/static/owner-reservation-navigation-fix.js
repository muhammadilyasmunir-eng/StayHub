(() => {
  ...
  const defaults={dateOf:'checkin',from:today(),until:dateAdd(today(),1),statuses:[],corporateCard:false,guestCommunication:false,pendingGuestRequest:false,invoiceRequired:false,invalidCard:false,updated:false,pending:false,guestOrBooking:'',activeDashboard:''};
  ...
  // Status-action date: prefer a dedicated cancellation/no-show timestamp, then the
  // record's status-change/update timestamp, and finally check-out as the safest
  // reservation-level fallback for older records that have no action timestamp.
  function statusEventDate(r,type){
    if(type==='canceled') return dateOnly(r.canceled_at||r.cancelled_at||r.cancellation_date||r.cancelled_date||r.status_changed_at||r.canceled_on||r.cancelled_on||r.updated_at||r.check_out||r.check_in);
    if(type==='no-show') return dateOnly(r.no_show_at||r.noshow_at||r.no_show_date||r.noshow_date||r.status_changed_at||r.no_show_on||r.noshow_on||r.updated_at||r.check_out||r.check_in);
    return '';
  }
  ...
  function dateMatch(r){
    ...
    if(state.dateOf==='canceled'){
      if(!isCancel(r))return false;
      const d=statusEventDate(r,'canceled');
      return !!d&&d>=from&&d<=until;
    }
    if(state.dateOf==='no-show'){
      if(!isNoShow(r))return false;
      const d=statusEventDate(r,'no-show');
      return !!d&&d>=from&&d<=until;
    }
    switch(state.dateOf){
  ...
  function filterRows(rows){
    ...
    return (rows||[]).filter(r=>{
      ...
      // Canceled/No-show are Date-of modes, not More filters. Their status is
      // already enforced by dateMatch, so never let stale saved filter state
      // interfere with the result.
      if(state.dateOf!=='canceled'&&state.dateOf!=='no-show'&&state.statuses.length){
        const s=normalize(r.status);
        const ok=state.statuses.some(x=>x==='ok'?isConfirmed(r):x==='canceled'?isCancel(r):x==='no-show'?isNoShow(r):s===normalize(x));
        if(!ok)return false;
      }
  ...
  function ensureUI(){
    ...
    document.querySelectorAll('#shReservationSearchV2Root input[data-f]').forEach(x=>x.checked=state.dateOf!=='canceled'&&state.dateOf!=='no-show'&&state.statuses.includes(x.dataset.f));
    document.getElementById('shV2Show').onclick=()=>{
      ...
      state.statuses=state.dateOf==='canceled'?[]:state.dateOf==='no-show'?[]:[...document.querySelectorAll('#shReservationSearchV2Root input[data-f]:checked')].map(x=>x.dataset.f);
  ...
  function openDashboard(filter){
    const t=today();state.activeDashboard=filter||'today';
    state.dateOf=filter==='today-checkout'?'checkout':filter==='stay-over'?'stay':filter==='cancel'?'canceled':filter==='no-show'?'no-show':'checkin';
    state.from=t;state.until=filter==='today-checkout'?t:(filter==='today-checkin'?t:filter==='stay-over'?t:t);
    state.statuses=[];state.corporateCard=false;state.guestCommunication=false;state.pendingGuestRequest=false;state.invoiceRequired=false;state.invalidCard=false;state.updated=false;state.pending=false;state.guestOrBooking='';persist();showReservations();render();
  }
