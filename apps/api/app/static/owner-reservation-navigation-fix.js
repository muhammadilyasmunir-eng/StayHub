(() => {
  ...
  // Normalize status labels such as "No Show", "No-Show", and "no_show"
  ...
  const defaults={dateOf:'reservation',from:today(),until:dateAdd(today(),1),statuses:[],corporateCard:false,guestCommunication:false,pendingGuestRequest:false,invoiceRequired:false,invalidCard:false,updated:false,pending:false,guestOrBooking:'',activeDashboard:''};
  let saved=null;try{saved=JSON.parse(sessionStorage.getItem('stayhub_reservation_search_v2')||'null')}catch{}
  const state=window.__stayhubReservationSearch=Object.assign({},defaults,saved&&typeof saved==='object'?saved:{});
  function persist(){try{sessionStorage.setItem('stayhub_reservation_search_v2',JSON.stringify(state))}catch{}}
  ...
  function dateMatch(r){
    ...
    switch(state.dateOf){
      ...
      case'stay':return dateIn(r)<=until&&dateOut(r)>=from;
      case'canceled':{
        if(!isCancel(r))return false;
        const d=dateOnly(r.canceled_at||r.cancelled_at||r.cancellation_date||r.cancelled_date||r.status_changed_at||r.canceled_on||r.cancelled_on||r.updated_at);
  ...
  function filterRows(rows){
    const q=state.guestOrBooking.trim().toLowerCase();
    return (rows||[]).filter(r=>{
      if(!dateMatch(r))return false;
      if(state.statuses.length){
        const s=normalize(r.status);
        const ok=state.statuses.some(x=>x==='ok'?isConfirmed(r):s===normalize(x));
        if(!ok)return false;
      }
  ...
  function style(){
    if(document.getElementById('shReservationSearchV2'))return;
    const s=document.createElement('style');s.id='shReservationSearchV2';
    s.textContent=`.sh-v2-search{border:1px solid #e1e6ee;border-radius:12px;background:#fff;padding:16px;margin-bottom:14px}.sh-v2-row{display:grid;grid-template-columns:180px 1fr 1fr auto;gap:12px;align-items:end}.sh-v2-field{display:flex;flex-direction:column;gap:6px}.sh-v2-field>label{font-size:11px;font-weight:800;color:#667085}.sh-v2-field input,.sh-v2-field select{height:40px;border:1px solid #d7dce5;border-radius:8px;padding:7px 10px;background:#fff}.sh-v2-more{margin-top:12px}.sh-v2-more summary{cursor:pointer;font-weight:800;color:#2563eb;user-select:none}.sh-v2-checks{display:flex;flex-wrap:wrap;gap:9px;margin-top:10px}.sh-v2-check{display:inline-flex;align-items:center;gap:6px;border:1px solid #e1e6ee;border-radius:8px;padding:7px 9px;font-size:12px;background:#fff}.sh-v2-actions{display:flex;gap:8px}.sh-v2-btn{height:40px;border:1px solid #d7dce5;border-radius:8px;padding:0 14px;background:#fff;font-weight:800;cursor:pointer}.sh-v2-btn.primary{background:#2563eb;border-color:#2563eb;color:#fff}.sh-v2-summary{display:flex;justify-content:space-between;align-items:center;padding:10px 2px;font-size:13px}.sh-v2-table{min-width:1120px}.sh-v2-table tbody tr{cursor:pointer}.sh-v2-table tbody tr:hover{background:#f8fbff}.sh-v2-status{display:inline-flex!important;align-items:center;border-radius:999px;padding:5px 10px;font-size:11px;font-weight:850;line-height:1;white-space:nowrap}.sh-v2-ok{background:#eaf8ef!important;color:#15803d!important}.sh-v2-cancel{background:#fff0f1!important;color:#b91c1c!important}.sh-v2-noshow{background:#f3e8ff!important;color:#7e22ce!important}.sh-v2-pending{background:#eef2ff!important;color:#4338ca!important}.sh-v2-empty{padding:45px;text-align:center;color:#667085}.sh-v2-empty b{display:block;color:#172033;margin-bottom:5px}@media(max-width:800px){.sh-v2-row{grid-template-columns:1fr 1fr}.sh-v2-actions{grid-column:1/-1}.sh-v2-btn{flex:1}}@media(max-width:520px){.sh-v2-row{grid-template-columns:1fr}}`;
  ...
  function ensureUI(){
    const sec=document.getElementById('reservations'),card=sec?.querySelector('.card');if(!sec||!card)return;
    style();let old=card.querySelector('.sh-res-toolbar');if(old)old.style.display='none';
    let root=document.getElementById('shReservationSearchV2Root');
    if(!root){
      root=document.createElement('div');root.id='shReservationSearchV2Root';root.className='sh-v2-search';
      root.innerHTML=`<div class="sh-v2-row"><div class="sh-v2-field"><label>Date of</label><select id="shV2DateOf"><option value="reservation">Reservation</option><option value="checkin">Check-in</option><option value="checkout">Check-out</option><option value="stay">Stay</option><option value="canceled">Canceled</option></select></div><div class="sh-v2-field"><label>From</label><input id="shV2From" type="date"></div><div class="sh-v2-field"><label>Until</label><input id="shV2Until" type="date"></div><div class="sh-v2-actions"><button class="sh-v2-btn primary" id="shV2Show" type="button">Show</button></div></div><details class="sh-v2-more"><summary>More filters</summary><div class="sh-v2-checks"><label class="sh-v2-check"><input type="checkbox" data-f="ok"> Ok</label><label class="sh-v2-check"><input type="checkbox" id="shV2Corporate"> Corporate card</label><label class="sh-v2-check"><input type="checkbox" id="shV2Communication"> Guest Communication</label><label class="sh-v2-check"><input type="checkbox" id="shV2PendingRequest"> Pending guest request</label><label class="sh-v2-check"><input type="checkbox" id="shV2Invoice"> Invoice required</label><label class="sh-v2-check"><input type="checkbox" id="shV2InvalidCard"> Invalid credit card</label><label class="sh-v2-check"><input type="checkbox" id="shV2Updated"> Updated</label><label class="sh-v2-check"><input type="checkbox" id="shV2Pending"> Pending</label><label class="sh-v2-check" style="flex:1;min-width:230px"><input type="checkbox" id="shV2GuestBooking"> Guest name or booking number</label><input id="shV2Query" type="search" placeholder="Enter guest name or booking number" style="height:34px;border:1px solid #d7dce5;border-radius:7px;padding:5px 9px;min-width:230px"></div></details></div>`;
    ...
    document.getElementById('shV2Show').onclick=()=>{
      state.dateOf=document.getElementById('shV2DateOf').value;state.from=document.getElementById('shV2From').value;state.until=document.getElementById('shV2Until').value;
      state.statuses=[...document.querySelectorAll('#shReservationSearchV2Root input[data-f]:checked')].map(x=>x.dataset.f);
      state.corporateCard=document.getElementById('shV2Corporate').checked;state.guestCommunication=document.getElementById('shV2Communication').checked;state.pendingGuestRequest=document.getElementById('shV2PendingRequest').checked;state.invoiceRequired=document.getElementById('shV2Invoice').checked;state.invalidCard=document.getElementById('shV2InvalidCard').checked;state.updated=document.getElementById('shV2Updated').checked;state.pending=document.getElementById('shV2Pending').checked;state.guestOrBooking=document.getElementById('shV2GuestBooking').checked?document.getElementById('shV2Query').value.trim():'';
      state.activeDashboard='';persist();render();
    };
  }
  function render(){
    ensureUI();const host=document.getElementById('reservationTable');if(!host)return;const rows=filterRows(window.data?.res||[]);
    const summary=document.getElementById('shV2Summary')||document.createElement('div');summary.id='shV2Summary';summary.className='sh-v2-summary';summary.innerHTML=`<b>${rows.length} reservation${rows.length===1?'':'s'}</b><span>${state.from||'—'} → ${state.until||'—'}</span>`;if(!summary.parentNode)host.parentNode.insertBefore(summary,host);
    host.innerHTML=rows.length?`<div class="table-wrap"><table class="table sh-v2-table"><thead><tr><th>Guest Name</th><th>Check-in</th><th>Check-out</th><th>Rooms</th><th>Booked on</th><th>Status</th><th>Price</th><th>Commission</th><th>Booking number</th></tr></thead><tbody id="resBody">${rows.map(r=>`<tr data-res-id="${esc(r.id)}"><td><b>${esc(r.guest_name||'Guest')}</b><small style="display:block;color:#667085;margin-top:4px">${esc(`${r.adults||0} adults${r.children?`, ${r.children} children`:''}`)}</small></td><td>${esc(dateOnly(r.check_in)||'—')}</td><td>${esc(dateOnly(r.check_out)||'—')}</td><td>${esc(r.room_type_name||r.room_number||'—')}</td><td>${esc(created(r)||'—')}</td><td>${statusHtml(r.status)}</td><td><b>${money(r.total_amount)}</b></td><td>${money(isCancel(r)?0:r.commission_amount)}</td><td><b>${esc(r.confirmation_no||r.booking_number||r.id||'—')}</b></td></tr>`).join('')}</tbody></table></div>`:'<div class="sh-v2-empty"><b>No reservations found</b>Try changing the selected dates or filters.</div>';
  }
  function openDashboard(filter){
    const t=today(),u=dateAdd(t,1);state.activeDashboard=filter||'reservation';
    if(filter==='today-checkout'){state.dateOf='checkout';state.from=t;state.until=t}
    else if(filter==='stay-over'){state.dateOf='stay';state.from=t;state.until=t}
    else if(filter==='cancel'){state.dateOf='canceled';state.from=t;state.until=t}
    else {state.dateOf='reservation';state.from=t;state.until=u}
    state.statuses=[];state.corporateCard=false;state.guestCommunication=false;state.pendingGuestRequest=false;state.invoiceRequired=false;state.updated=false;state.pending=false;state.guestOrBooking='';persist();showReservations();render();
  }
  ...
  const originalNav=window.navTo;window.navTo=function(id,filter){if(id==='reservations'){openDashboard(filter||'reservation');return}return typeof originalNav==='function'?originalNav.apply(this,arguments):undefined};
  ...
  const originalShow=window.show;window.show=function(id,btn){const result=originalShow?.apply(this,arguments);if(id==='reservations'){setTimeout(()=>{openDashboard('reservation')},80)}return result};
