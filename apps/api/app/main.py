...
def ensure_schema():
    ...
    with engine.begin() as conn:
        conn.execute(text("ALTER TABLE terms_documents ADD COLUMN IF NOT EXISTS document_type VARCHAR(40) DEFAULT 'terms'"))
        conn.execute(text("UPDATE terms_documents SET document_type='terms' WHERE document_type IS NULL"))
        conn.execute(text("ALTER TABLE hotels ADD COLUMN IF NOT EXISTS commission_percent NUMERIC(5,2)"))
...
def public_website():
    ...
    html=html.replace('href="/static/public/login.html"','href="/customer-login"')
...
def owner_registration():
    path=STATIC_DIR/"public"/"owner-register-v2.html"; html=path.read_text(encoding="utf-8")
    scripts='<script src="/static/public/owner-register-operational-ui.js?v=1"></script><script src="/static/public/owner-property-edit.js?v=1"></script><script src="/static/public/owner-register-room-pricing.js?v=1"></script><script src="/static/date-defaults.js?v=3"></script>'
